...
较上一版改进:
  - 首次创建
"""
from constants import TEST_DATA_DIR, OUTPUT_DIR
import glob
...
config.file_operation_params = {
    ...
    "output_dir": f"{OUTPUT_DIR}/file_operation"
}
# 文件保存任务
config.file_saver_params = {
    "save_dir": f"{OUTPUT_DIR}/saved_files"
}
# 图像显示任务
config.image_display_params = {
    # 直接读取 file_operation 生成的 .npy 文件列表
    "paths": glob.glob(f"{OUTPUT_DIR}/file_operation/*.npy"),
    # 要展示的波段（基于 1），默认可写 [1,2,3]
    "bands": [1, 2, 3],
    # 保存 PNG 的目录
    "output_dir": f"{OUTPUT_DIR}/display"
}
# 图像处理任务
config.image_processing_params = {
    "paths": glob.glob(f"{OUTPUT_DIR}/file_operation/*.npy"),
    "methods": ["equalization", "stretch"],
    "output_dir": f"{OUTPUT_DIR}/processed",
    "options": {
...
config.vector_processing_params = {
    # 将单文件改成列表
    "input_paths": [
        f"{TEST_DATA_DIR}/sample.shp",
        # 可以加更多 .shp
    ],
    # 对应批量接口里的 output_dir
    "output_dir": f"{OUTPUT_DIR}/vector",
    ...
    "operations": [
        ...
        # 再做裁剪
        # {"type": "clip",      "params": {"mask": f"{TEST_DATA_DIR}/mask.shp"}},
    ],
    # 可选的加载/保存参数
    "options": {
        "load": None,
...
config.classification_params = {
    ...
    "data": {
        "features": f"{OUTPUT_DIR}/processed/features.npy",
        "labels":   f"{OUTPUT_DIR}/processed/labels.npy"
    },
    ...
    # 分类结果保存路径
    "class_map_path": f"{OUTPUT_DIR}/processed/class_map.npy",
    # 额外标记模型名（可选，run() 中已兼容）
    "model": model
}
# 精度评估任务
config.evaluation_params = {
    "class_map_path": f"{OUTPUT_DIR}/processed/class_map.npy",
    "roi_mask_path": f"{TEST_DATA_DIR}/mask.npy",
